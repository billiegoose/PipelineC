...
def NORMALIZE_KNOWN_ZERO_DELAY_SUBMODULE_DELAYS(logic, parser_state):
    ...
    child so callers can retain the old hard-error behavior for missing timing.
    """
    for sub_inst in logic.submodule_instances:
        ...
        if sub_func_logic.delay is None:
            if LOGIC_IS_ZERO_DELAY(sub_func_logic, parser_state, allow_none_delay=True):
...
def BUILD_FIXED_AUTO_PIPELINE_TIMING_PARAMS(parser_state):
    ...
    if not any(region.constraint.latency > 0 for region in regions):
        return None
    global SYN_TOOL
    previous_tool = SYN_TOOL
    PART_SET_TOOL(parser_state.part, allow_fail=True)
    borrowed_tool = SYN_TOOL is None or OPEN_TOOLS.YOSYS_JSON_ONLY
    if borrowed_tool:
        print(
